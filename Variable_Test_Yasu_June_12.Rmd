---
title: "Variable_Test_Yasu_June_12"
...
panel_reduced_val <- panel_reduced_val[!is.na(BTM),c("Id","ym","MV.USD","RET.USD","B/M")]
setorder(panel_reduced_val,ym,`B/M`) 
...
```{r}
cum.ret.data <- panel_reduced %>%
  group_by(Id)%>%
  mutate(CUM.RET.USD.3=rollapply(RET.USD,3,sum,align='right',fill=NA)) %>%
  mutate(CUM.RET.USD.6=rollapply(RET.USD,6,sum,align='right',fill=NA)) %>%
  mutate(CUM.RET.USD.12=rollapply(RET.USD,12,sum,align='right',fill=NA)) %>%
  ungroup
```
