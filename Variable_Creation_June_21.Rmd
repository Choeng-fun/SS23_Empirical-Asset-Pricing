...
panel_reduced_CEI[,lag_5 := shift(MV.USD,-1), by = "Id"]
panel_reduced_NSI[,lag_4:=shift(MV.USD,-2), by = "Id"]
panel_reduced_NSI[,lag_3:=shift(MV.USD,-3), by = "Id"]
panel_reduced_NSI[,lag_2:=shift(MV.USD,-4), by = "Id"]
panel_reduced_NSI[,lag_1:=shift(MV.USD,-5), by = "Id"]
panel_reduced_NSI[,lag_12:=shift(MV.USD,-6), by = "Id"]
panel_reduced_NSI[,lag_11:=shift(MV.USD,-7), by = "Id"]
panel_reduced_NSI[,lag_10:=shift(MV.USD,-8), by = "Id"]
panel_reduced_NSI[,lag_9:=shift(MV.USD,-9), by = "Id"]
panel_reduced_NSI[,lag_8:=shift(,-10), by = "Id"]
panel_reduced_NSI[,lag_7:=shift(NSI_m,-11), by = "Id"]
...
Factor_data_ff5 <- panel_reduced_REM %>% 
  ...
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`)%>% 
  full_join(panel_reduced_GPA, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`) %>% 
  full_join(panel_reduced_OPBE, by = c("Id", "ym", "MV.USD", "RET.USD")) %>%
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`) %>% 
  full_join(panel_reduced_NOA, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA) %>% 
  full_join(panel_reduced_OA, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA) %>% 
  full_join(panel_reduced_inv, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG) %>% 
  full_join(panel_reduced_NSI, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI) %>%
  full_join(panel_reduced_IA, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI, `I/A`) %>%
  full_join(panel_reduced_MV, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI, `I/A`, MV) %>% 
  full_join(panel_reduced_momentum, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI, `I/A`, MV, Mom_12_2)
```
