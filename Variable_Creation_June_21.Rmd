...
load("/Users/kojimadamsimith/Desktop/Summer Semester/Advanced Seminar/GBR/Original_Dataset/GBR_WS_yearly.RData")
```
...
```{r}
load("~/Desktop/Summer Semester/Advanced Seminar/GBR/SS23_Empirical-Asset-Pricing/panel_country_June30.RData")
load("~/Desktop/Summer Semester/Advanced Seminar/GBR/SS23_Empirical-Asset-Pricing/DTB6_monthly_July01.RData")
...
check <- panel_reduced_val %>% 
  select(Id, year, month, ym, MV.USD, MV, MV_last_dec, `B/M`, `B/Mm`)
...
# coef_60 colums order is diffirent from coef_12 and coef_36
names(panel_reduced_alpha_5) <- c("Id", "ym", "MV.USD", "RET.USD", "coef_5", "Alpha_5", "Beta_5")
...
panel_reduced_sd <- panel_reduced %>%
  group_by(Id)%>%
  mutate(SD=rollapply(RET.USD,36,sd,align='right',fill=NA)) %>%
  ungroup %>% 
...
```{r}
#Select only necessary variables from the comprehensive data set 
panel_reduced_TO <- panel_country[!is.na(RET.USD),c("Id","ym","MV.USD","RET.USD","WC05651", "WC05301","NOSH")]
...
#WC05651 Common Shares Traded - Annual
# NOSH the number of shares outstanding
panel_reduced_TO <- panel_reduced_TO[,Lagged_Out := shift(NOSH, n = 12)]
panel_reduced_TO[,Avg_Out := (NOSH + Lagged_Out)/2]
...
Factor_data_ff5_FM <- panel_reduced_REM %>% 
  ...
  select(Id, ym, MV.USD, REM.USD,RE.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI, CEI) %>%
  full_join(panel_reduced_IA, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD,RE.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI,CEI, `I/A`) %>%
  full_join(panel_reduced_MV, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD,RE.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI,CEI, `I/A`, MV) %>% 
  full_join(panel_reduced_momentum, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD,RE.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI,CEI, `I/A`, MV, Mom_12_2) %>% 
 full_join(panel_reduced_beta_1 ,by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD,RE.USD, RET.USD, Beta_3, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI,CEI, `I/A`, MV, Mom_12_2, Beta_1) %>% 
  full_join(panel_reduced_beta_5 ,by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, REM.USD,RE.USD, RET.USD, Beta_1, Beta_3, Beta_5, `B/M`, `B/Mm`, `E/Pm`,`C/Pm`, `GP/A`,`OP/BE`, NOA, OA, AG, NSI, CEI, `I/A`, MV, Mom_12_2)
```
...
##Factor data MSCI
```{r}
Factor_data_MSCI <- panel_reduced_REM %>% 
  full_join(panel_reduced_val, by = c("Id", "ym", "RET.USD")) %>% 
  ...
  full_join(panel_reduced_CEI, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  full_join(panel_reduced_IA, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, RET.USD, REM.USD, RE.USD, `B/M`, `B/Mm`,`E/P`, `E/Pm`,`C/P`, `C/Pm`, `EBITDA/EV`, Log_MV, RSI, Alpha_1, Alpha_3, Alpha_5, Mom_12_2, DY, ROE, ROA, Beta_1, Beta_3, Beta_5, Sigma_1, Sigma_3, Sigma_5, SD, `T/O`, EPS, SPS, MV, `GP/A`,`OP/BE`,NOA,OA,AG,NSI,CEI,`I/A`)
```


```{r}
save(Factor_data_MSCI, file = "/Users/kojimadamsimith/Desktop/Summer Semester/Advanced Seminar/GBR/Factor_data_MSCI.RData")
```
