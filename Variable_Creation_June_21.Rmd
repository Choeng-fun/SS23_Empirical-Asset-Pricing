...
check_copy[,MV_lag_1:=lag(check_copy$MV,1)]
check_copy[,MV_lag_2:=lag(check_copy$MV,2)]
check_copy[,MV_lag_3:=lag(check_copy$MV,3)]
check_copy[,MV_lag_4:=lag(check_copy$MV,4)]
check_copy[,MV_lag_5:=lag(check_copy$MV,5)]
check_copy[,MV_lag_6:=lag(check_copy$MV,6)]
...
check_copy[month==1,MV_last_dec:=MV_lag_1]
check_copy[month==2,MV_last_dec:=MV_lag_2]
check_copy[month==3,MV_last_dec:=MV_lag_3]
check_copy[month==4,MV_last_dec:=MV_lag_4]
check_copy[month==5,MV_last_dec:=MV_lag_5]
check_copy[month==6,MV_last_dec:=MV_lag_6]
...
panel_reduced_val <- panel_reduced_val[,`B/M`:= Book_Value/(MV*1000)]
...
panel_reduced_OPBE <- panel_reduced_OPBE[,`OP/BE`:=(WC01001-WC01051-WC01101)/Bool_Equity] 
...
Factor_data_ff5 <- coef_36 %>% 
  ...
  select(Id, ym, MV.USD, RET.USD, Beta_3, `B/M`)%>% 
  full_join(panel_reduced_ROE, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, MV.USD, RET.USD, Beta_3, `B/M`, ROE) %>% 
  full_join(panel_reduced_inv, by = c("Id", "ym", "MV.USD", "RET.USD")) %>%
  select(Id, ym, MV.USD, RET.USD, Beta_3, `B/M`, ROE, AG) %>% 
  full_join(panel_reduced_momentum, by = c("Id", "ym", "MV.USD", "RET.USD")) %>% 
  select(Id, ym, RET.USD, Beta_3, MV.USD, `B/M`, ROE, AG, Mom_12_2)
```
