---
title: "Variable_Test_Portfolio_Sort_Weighted_Mean_June_21"
...
portfolio_returns_val_w <- panel_reduced_val %>% 
...
portfolio_returns_val_w <- as.data.table(portfolio_returns_val_w)
portfolio_returns_val_w[,t.test(Ret_val)]
```
...
```{r}
portfolio_returns_SMB_w <- panel_reduced_SMB %>% # this operator nests functions
  group_by(ym,pf.size)%>% # do "everything" for the groups specified here
  summarize(ret.port = weighted.mean(RET, coalesce(MV, 0), na.rm = T))%>% # vw returns using lagged mcap
  spread(pf.size,ret.port) %>% # create one column for each group
  mutate(SMB = Small-Big)
```

```{r}
portfolio_returns_SMB_w <- as.data.table(portfolio_returns_SMB_w)
portfolio_returns_SMB_w[,t.test(SMB)]
```
...
panel_reduced_momentum <- panel_reduced_momentum[,Momentum_12_2:=((100+Before3)*(100+Before4)*(100+Before5)*(100+Before6)*(100+Before7)*(100+Before8)*(100+Before9)*(100+Before10)*(100+Before11)*(100+Before12))^(1/10)-100]
...
portfolio_returns_momentum_w <- panel_reduced_momentum %>% 
  group_by(ym,Decile)%>% # do "everything" for the groups specified here
  summarize(ret.port = weighted.mean(RET.USD, coalesce(MV.USD, 0), na.rm = T)) %>% # calculate mean return for each group
  spread(Decile,ret.port) %>% # create one column for each group
...
```{r}
# Test if the momentum return is significantly different from 0
portfolio_returns_momentum_w <- as.data.table(portfolio_returns_momentum_w)
portfolio_returns_momentum_w[,t.test(Ret_Momentum)]
```
...
```{r}
# Long the past winners (stocks in the 10th decile), short the past losers(stocks in the 1st decile), and get the momentum return
portfolio_returns_rsi_w <- panel_reduced_rsi %>% 
  group_by(ym,Decile)%>% # do "everything" for the groups specified here
  summarize(ret.port = weighted.mean(RET.USD, coalesce(MV.USD, 0), na.rm = T)) %>% # calculate mean return for each group
  spread(Decile,ret.port) %>% # create one column for each group
  mutate(Ret_rsi = `10`-`1`)
...
```{r}
# Test if the momentum return is significantly different from 0
portfolio_returns_rsi_w <- as.data.table(portfolio_returns_rsi_w)
portfolio_returns_rsi_w[,t.test(Ret_rsi)]
```
###Historical Alpha 
####1-Year
...
```{r}
# Long the past winners (stocks in the 10th decile), short the past losers(stocks in the 1st decile), and get the momentum return
portfolio_returns_alpha_w <- panel_reduced_alpha %>% 
  group_by(ym,Decile)%>% # do "everything" for the groups specified here
  summarize(ret.port = weighted.mean(RET.USD, coalesce(MV.USD, 0), na.rm = T)) %>% # calculate mean return for each group
  spread(Decile,ret.port) %>% # create one column for each group
...
```{r}
# Test if the momentum return is significantly different from 0
portfolio_returns_alpha_w <- as.data.table(portfolio_returns_alpha_w)
portfolio_returns_alpha_w[,t.test(Ret_alpha)]
```
