...
```{r}
Factor_data_MSCI_r <- Factor_data_MSCI[complete.cases(Factor_data_MSCI)]
```
...
panel_reduced_EE <- panel_reduced_CPm[`EBITDA/EV` > 0]
...
setorder(panel_reduced_lnMV,ym,RSI) 
...
```{r}
# Order stocks based on the past performance, divide them into 10 deciles, and filter the 1st and 10th decile.
panel_reduced_alpha_3 <- Factor_data_MSCI[!is.na(Alpha_3),c("Id","ym","MV.USD","RET.USD","Alpha_1")]
...
```{r}
# Test if the momentum return is significantly different from 0
portfolio_returns_alpha_5 <- as.data.table(portfolio_returns_alpha_1)
...
setorder(panel_reduced_ROE,ym,ROA) 
panel_reduced_ROA <- panel_reduced_ROA[, Decile := ntile(ROE, 10), by = ym]
...
```{r}
# Order stocks based on the past performance, divide them into 10 deciles, and filter the 1st and 10th decile.
panel_reduced_beta_5 <- Factor_data_MSCI[!is.na(Beta_5),c("Id","ym","MV.USD","RET.USD","Beta_1")]
...
panel_reduced_sps <- panel_reduced_sps[, Decile := ntile(EPS, 10), by = ym]
