...
stock_list_20_6_cum_ym <- stock_list_20_6_cum %>%
  mutate(ym=as.yearmon(ym,"%Y.%m"))
time_series_return_df <- stock_list_20_6_cum_ym[, c("Id", "ym", "RET.USD")]
...
```{r}
stock_number <- 20
unique_dates <- unique(stock_list_20_6_cum_ym$ym)
for (date in unique_dates) {
  matching_rows <- filter(df, ym == date)
  print(matching_rows)
