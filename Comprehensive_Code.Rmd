...
```{r}
# save a table that contain SMB, etc.


portfolio_returns_SMB_reduced <- portfolio_returns_SMB %>% dplyr::select(c("ym","SMB")) 
portfolio_returns_HML_reduced <- portfolio_returns_HML %>% dplyr::select(c("ym","HML")) 
portfolio_returns_CMA_reduced <- portfolio_returns_CMA %>% dplyr::select(c("ym","CMA")) 
portfolio_returns_RMW_reduced <- portfolio_returns_RMW %>% dplyr::select(c("ym","RMW")) 
portfolio_returns_WML_reduced <- portfolio_returns_WML %>% dplyr::select(c("ym","WML")) 

StyleAnalysis_FF_and_Mom_Set <- full_join(portfolio_returns_SMB_reduced,portfolio_returns_HML_reduced,by="ym") %>% 
  full_join(.,portfolio_returns_CMA_reduced,by="ym") %>%  
  full_join(.,portfolio_returns_RMW_reduced,by="ym") %>% 
  full_join(.,portfolio_returns_WML_reduced,by="ym") %>% 
  filter(!is.na(SMB)&!is.na(HML)&!is.na(CMA)&!is.na(RMW)&!is.na(WML))

save(StyleAnalysis_FF_and_Mom_Set,file="/Users/yuruchen/Desktop/Empirical Asset Pricing/Repo/Style Analysis/StyleAnalysis_FF_and_Mom_Set.Rdata")
```
...
lag_order <- VARselect(example, lag.max = 10, type = "both")$selection[1]
print(lag_order)

lagselect$selection$bic
...
## 10. Tangent Portfolio(weighting and rebalancing)
