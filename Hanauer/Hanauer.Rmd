...
```{r}
Factor_data_ff5_FM_temp1 <- Factor_data_ff5_FM[str_sub(ym, -2, -2)=="."]
Factor_data_ff5_FM_temp1 <- Factor_data_ff5_FM_temp1[, ym:=paste(str_sub(ym, 1, 5),"0",str_sub(ym, -1), sep="")]
Factor_data_ff5_FM_temp1 <- janitor::remove_empty(Factor_data_ff5_FM_temp1, which = "cols")
Factor_data_ff5_FM_temp2 <- Factor_data_ff5_FM[str_sub(ym, -2, -2)!="." & strtoi(str_sub(ym, -2)) < 10]
Factor_data_ff5_FM_temp2 <- janitor::remove_empty(Factor_data_ff5_FM_temp2, which = "cols")
```


```{r}
Factor_data_ff5_FM_temp1_2 <- merge(Factor_data_ff5_FM_temp2, Factor_data_ff5_FM_temp1, all=T)
Factor_data_ff5_FM_temp3 <- Factor_data_ff5_FM[str_sub(ym, -2, -2)!="." & strtoi(str_sub(ym, -2)) >= 10]
Factor_data_ff5_FM_temp <- rbind(Factor_data_ff5_FM_temp3, Factor_data_ff5_FM_temp1_2)
```

```{r}
beta <- Factor_data_ff5_FM_temp %>% 
  group_by(ym) %>% 
  summarise(Beta = mean(Beta_3, na.rm=T)) %>%
  as.data.frame()
  ungroup()
```

```{r}
regression_df <- as.data.table(Factor_data_ff5_FM_temp)
```
...
test <- regression_df %>% 
  ...
  # drop_na()
  # group_by(ym) %>% 
  # summarise(gamma_zero=lm(RE.USD~Beta_3+`B/M`+`OP/BE`+AG+MV)$coefficient[1]) %>%
  # ungroup()
```

```{r}
panel_a_fun <- function() 
panel_a <- Factor_data_ff5_FM_temp[!is.na(Beta_3) & !is.infinite(Beta_3) & !is.na(`B/M`) & !is.infinite(`B/M`) & !is.na(`OP/BE`) & !is.infinite(`OP/BE`) & !is.na(AG) & !is.infinite(AG) & !is.na(MV) & !is.infinite(MV),
...
```{r}
panel_b <- Factor_data_ff5_FM_temp[!is.na(Beta_3) & !is.infinite(Beta_3) & !is.na(`B/M`) & !is.infinite(`B/M`) & !is.na(`OP/BE`) & !is.infinite(`OP/BE`) & !is.na(AG) & !is.infinite(AG) & !is.na(MV) & !is.infinite(MV) & !is.na(Mom_12_2) & !is.infinite(Mom_12_2),
...
```{r}
panel_c <- Factor_data_ff5_FM_temp[!is.na(Beta_3) & !is.infinite(Beta_3) & !is.na(`B/M`) & !is.infinite(`B/M`) & !is.na(`B/Mm`) & !is.infinite(`B/Mm`) & !is.na(`OP/BE`) & !is.infinite(`OP/BE`) & !is.na(AG) & !is.infinite(AG) & !is.na(MV) & !is.infinite(MV) & !is.na(Mom_12_2) & !is.infinite(Mom_12_2),
...
```{r}
panel_c2 <- Factor_data_ff5_FM_temp[!is.na(Beta_3) & !is.infinite(Beta_3) & !is.na(`B/M`) & !is.infinite(`B/M`) & !is.na(`C/Pm`) & !is.infinite(`C/Pm`) & !is.na(`OP/BE`) & !is.infinite(`OP/BE`) & !is.na(AG) & !is.infinite(AG) & !is.na(NSI) & !is.infinite(NSI) & !is.na(`OP/BE`) & !is.infinite(`OP/BE`) & !is.na(MV) & !is.infinite(MV) & !is.na(Mom_12_2) & !is.infinite(Mom_12_2),
...
```{r}
panel_d <- Factor_data_ff5_FM_temp[!is.na(Beta_3) & !is.infinite(Beta_3) & !is.na(`GP/A`) & !is.infinite(`GP/A`) & !is.na(`OP/BE`) & !is.infinite(`OP/BE`) & !is.na(NOA) & !is.infinite(NOA) & !is.na(OA) & !is.infinite(OA) & !is.na(AG) & !is.infinite(AG) & !is.na(MV) & !is.infinite(MV) & !is.na(Mom_12_2) & !is.infinite(Mom_12_2),
