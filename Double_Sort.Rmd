...
Factor_data_ff5 <- as.data.table(Factor_data_ff5)
Factor_data_ff5[,month:=as.numeric(substr(ym,6,7))]
Factor_data_ff5[,year:=as.numeric(substr(ym,1,4))]
Factor_data_ff5[,hcjun := ifelse(month>=7,year,year-1)]
colnames(Factor_data_ff5)[6] <- "BM"
...
Factor_data_ff5 <- Factor_data_ff5[complete.cases(Factor_data_ff5)]

setorder(Factor_data_ff5,ym,-MV.USD) 
hlpvariable_size <-  Factor_data_ff5[month==7 & !is.na(MV.USD),
                                .(pf.size = ifelse((cumsum(MV.USD)/sum(MV.USD))>=0.9,"Small","Big"),Id),
                                by=year]

# Merge the size portfolio allocation back from July Y to June Y+1
panel_country <- merge(Factor_data_ff5,hlpvariable_size,
                       by.x=c("hcjun","Id"),
                       by.y=c("year","Id"))
```
...
portfolio_returns_SMB <- portfolio_returns_SMB[,SMB_average:=
                                                 (SMB_hml+SMB_rmw+SMB_cma+SMB_wml)/4]

portfolio_returns_SMB[,t.test(SMB_average)]
```

In conclusion, significant variables include SMB, HML, CMA, and WML
