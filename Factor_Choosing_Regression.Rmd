---
title: "Factor_Choosing_Regression"
author: "Yasu"
date: '2023-07-04'
...
```{r}
load("/Users/kojimadamsimith/Desktop/Summer Semester/Advanced Seminar/GBR/Univariate_Sort_Set.RData")
...
```{r}
panel_k <- Univariate_Sort_Set[!is.na(`B/Mm`) & !is.infinite(`B/Mm`) & !is.na(`E/Pm`) & !is.infinite(`E/Pm`) &!is.na(`C/Pm`) & !is.infinite(`C/Pm`)& !is.na(`Log_MV`) & !is.infinite(`Log_MV`) & !is.na(RSI) & !is.infinite(RSI) & !is.na(Alpha_3) & !is.infinite(Alpha_3) & !is.na(Mom_12_2) & !is.infinite(Mom_12_2) & !is.na(ROE) & !is.infinite(ROE) & !is.na(NOA) & !is.infinite(NOA) & !is.na(CEI) & !is.infinite(CEI) & !is.na(Sigma_3) & !is.infinite(Sigma_3) & !is.na(SD) & !is.infinite(SD),
```

```{r}
panel_k <- Univariate_Sort_Set[!is.na(`E/Pm`) & !is.infinite(`E/Pm`) & !is.na(`Log_MV`) & !is.infinite(`Log_MV`) & !is.na(RSI) & !is.infinite(RSI) & !is.na(Alpha_3) & !is.infinite(Alpha_3) & !is.na(Mom_12_2) & !is.infinite(Mom_12_2) & !is.na(ROE) & !is.infinite(ROE) & !is.na(CEI) & !is.infinite(CEI) & !is.na(Sigma_3) & !is.infinite(Sigma_3),
                                            .(gamma_zero=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[1],
                                              gamma_Sigma_3=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[2],
                                              gamma_EPm=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[3],
                                              gamma_ROE=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[4],
                                              gamma_CEI=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[5],
                                              gamma_Log_MV=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[6],
                               gamma_Alpha_3=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[7],

gamma_RSI=lm(ExRET.USD~Sigma_3+`E/Pm`+`ROE`+CEI+Log_MV+Alpha_3+RSI)$coefficient[8]),
                              by=ym]


panel_k[,t.test(gamma_zero)]
panel_k[,t.test(gamma_Sigma_3)]
panel_k[,t.test(gamma_EPm)]
panel_k[,t.test(gamma_ROE)]
panel_k[,t.test(gamma_CEI)]
panel_k[,t.test(gamma_Log_MV)]
panel_k[,t.test(gamma_Alpha_3)]
panel_k[,t.test(gamma_RSI)]
```


```{r}
panel_a <- Univariate_Sort_Set[!is.na(Beta_3) & !is.infinite(Beta_3) & !is.na(`B/M`) & !is.infinite(`B/M`) & !is.na(`OP/BE`) & !is.infinite(`OP/BE`) & !is.na(AG) & !is.infinite(AG) & !is.na(MV) & !is.infinite(MV),
                                            .(gamma_zero=lm(ExRET.USD~Beta_3+`B/M`+`OP/BE`+AG+MV)$coefficient[1],
                                              gamma_beta=lm(ExRET.USD~Beta_3+`B/M`+`OP/BE`+AG+MV)$coefficient[2],
                                              gamma_bm=lm(ExRET.USD~Beta_3+`B/M`+`OP/BE`+AG+MV)$coefficient[3],
                                              gamma_opbe=lm(ExRET.USD~Beta_3+`B/M`+`OP/BE`+AG+MV)$coefficient[4],
                                              gamma_ag=lm(ExRET.USD~Beta_3+`B/M`+`OP/BE`+AG+MV)$coefficient[5],
                                              gamma_mv=lm(ExRET.USD~Beta_3+`B/M`+`OP/BE`+AG+MV)$coefficient[6]),
                              by=ym]
