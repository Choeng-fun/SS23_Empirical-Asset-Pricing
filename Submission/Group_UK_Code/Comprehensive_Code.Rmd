...
load("DTB6_monthly_July01.RData")
...
```{r}
# Please run the first Univariate_Sort_Set in 4.1 Univariate Sort Dataset Preparation
...
Univariate_Sort_Set <- panel_reduced_MRP %>% 
  ...
  dplyr::select(Id, ym, MV.USD, RET.USD, MRP.USD, ExRET.USD, `B/M`, `B/Mm`,`E/P`, `E/Pm`,`C/P`, `C/Pm`, `EBITDA/EV`, Log_MV, RSI, Alpha_3, Mom_12_2, DY, ROE, ROA, Beta_3, Sigma_3, SD, EPS, SPS, MV, `GP/A`,`OP/BE`,NOA,OA,AG,NSI,CEI,`I/A`)
...
### 7.1 Selected Factors: Sigma_3, B/Mm, CEI, Log_MV, Alpha_3, RSI
```{r}
...
data_lewellen_6 <- cbind(coef_cross_sectional_6, mean36 = rollmeanr(coef_cross_sectional_6[, ..cols_6], 36, fill = NA))
data_lewellen_6 <- cbind(data_lewellen_6, cummean = cumsum(coef_cross_sectional_6[, ..cols_6]) / seq_along(data_lewellen_6[, ym]))
```

### 7.2 Selected Factors: Log_MV, Alpha_3, RSI
...
t6 <- DS.monthly %>%
  full_join(t5, by = c("Id", "ym")) %>% 
  dplyr::select(Id, ym, weight, RET.USD)
...
# Average number of stock
num_3_var <- t7_3_var_20 %>% 
  filter(weight != 0) %>% 
  group_by(ym) %>% 
  summarise(n = n())

print(mean(num_3_var$n))
...
# Average number of stock
num_5_var <- t7_5_var_20 %>% 
  filter(weight != 0) %>% 
  group_by(ym) %>% 
  summarise(n = n())

print(mean(num_5_var$n))
...
mean(annual_market_ret$ANNUAL_MKT_RET.USD)
sd(annual_market_ret$ANNUAL_MKT_RET.USD)
print(mean(annual_market_ret$ANNUAL_MKT_RET.USD)/sd(annual_market_ret$ANNUAL_MKT_RET.USD))

# value at risk
confidence_level <- 0.95
VaR <- quantile(annual_market_ret$ANNUAL_MKT_RET.USD, 1 - confidence_level)
print(VaR)
```
...
annual_3_mean_performance <- annual_3_mean_performance %>%
  ...
  ungroup() %>%
  mutate(ANNUAL_RET.USD=(PROD-1)*100) %>%
  dplyr::select(year, ANNUAL_RET.USD)
...
# value at risk
confidence_level <- 0.95
VaR <- quantile(annual_3_mean_performance$ANNUAL_RET.USD, 1 - confidence_level)
print(VaR)
```
...
# value at risk
confidence_level <- 0.95
VaR <- quantile(annual_3_cum_performance$ANNUAL_RET.USD, 1 - confidence_level)
print(VaR)
```
...
mean(annual_5_var_performance$ANNUAL_RET.USD)
sd(annual_5_var_performance$ANNUAL_RET.USD)
print(mean(annual_5_var_performance$ANNUAL_RET.USD)/sd(annual_5_var_performance$ANNUAL_RET.USD))
```
...
# value at risk
confidence_level <- 0.95
VaR <- quantile(annual_5_var_performance$ANNUAL_RET.USD, 1 - confidence_level)
print(VaR)
...
mean(annual_3_var_performance$ANNUAL_RET.USD)
sd(annual_3_var_performance$ANNUAL_RET.USD)
print(mean(annual_3_var_performance$ANNUAL_RET.USD)/sd(annual_3_var_performance$ANNUAL_RET.USD))
```
...
# value at risk
confidence_level <- 0.95
VaR <- quantile(annual_3_var_performance$ANNUAL_RET.USD, 1 - confidence_level)
print(VaR)
```
...
merge.data <- merge.data %>%
  rename(
    `X3_cum` = X3_cum_cumulative_return,
    `X3_var` = X3_var_cumulative_return,
    `X3_roll` = X3_mean_cumulative_return,
    `X5_var` = X5_var_cumulative_return,
    `X6_roll` = X6_mean_cumulative_return,
    `X6_cum` = X6_cum_cumulative_return,
    `X6_roll(equal weight)` = X6_equal_mean_cumulative_return
  )
...
plot.data_equal <- merge.data %>%
  dplyr::select(date,
    `X6_roll`,
    `X6_roll(equal weight)`,
    Benchmark) %>%
  rename(`X6_roll(tangent portfolio)` = `X6_roll`) %>% 
  gather(key = "variable", value = "value", -date)%>% 
  rename(Strategy = variable)

plot.data_cum$Strategy <- factor(
  plot.data_cum$Strategy ,
  levels = c(
    "X6_roll",
    "X3_roll",
    "X6_cum",
...
plot.data_equal$Strategy <- factor(
  plot.data_equal$Strategy ,
  levels = c(
    "X6_roll(tangent portfolio)",
    "X6_roll(equal weight)",
    "Benchmark"
  )
)

ggplot(plot.data_cum, aes(x = date, y = value)) + 
...
ggplot(plot.data_equal, aes(x = date, y = value)) + 
  geom_line(aes(color = Strategy), size=0.9)+
  theme(
    # panel.background = element_rect(fill='transparent'), #transparent panel bg
    plot.background = element_rect(fill='transparent', color=NA), #
